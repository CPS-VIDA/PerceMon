...
BBox::BBox(double xmin_, double xmax_, double ymin_, double ymax_,
           bool lopen_, bool ropen_, bool topen_, bool bopen_)
    : xmin(xmin_), xmax(xmax_), ymin(ymin_), ymax(ymax_),
      lopen(lopen_), ropen(ropen_), topen(topen_), bopen(bopen_) {}

BBox::BBox(const datastream::BoundingBox& bbox)
    : xmin(bbox.xmin), xmax(bbox.xmax), ymin(bbox.ymin), ymax(bbox.ymax),
      lopen(false), ropen(false), topen(false), bopen(false) {}

[[nodiscard]] auto BBox::area() const -> double {
    return (xmax - xmin) * (ymax - ymin);
}

[[nodiscard]] auto BBox::is_closed() const -> bool {
    return !(lopen || ropen || topen || bopen);
}

[[nodiscard]] auto BBox::is_open() const -> bool {
    return lopen || ropen || topen || bopen;
}
...
void Union::insert(const BBox& bbox) {
    regions.insert(bbox);
}

void Union::merge(const Union& other) {
    for (const auto& bbox : other.regions) {
        regions.insert(bbox);
    }
}
...
auto intersection_of(const Union& a, const BBox& b) -> Region {
    ...
    for (const BBox& bbox : a) {
        ...
        if (auto box_p = std::get_if<BBox>(&int_box)) {
    ...
    if (intersect_set.size() == 1) {
        return intersect_set.back();
    }
...
auto union_of(const BBox& a, const BBox& b) -> Region {
    ...
    if ((a.xmin <= b.xmin && b.xmax <= a.xmax) &&
        (a.ymin <= b.ymin && b.ymax <= a.ymax)) {
        // b is inside a
    ...
    if ((b.xmin <= a.xmin && a.xmax <= b.xmax) &&
        (b.ymin <= a.ymin && a.ymax <= b.ymax)) {
        // a is inside b
...
auto complement_of(BBox bbox, const BBox& universe) -> Region {
    ...
    // Check if box is out of bounds
    {
        ...
        if (std::holds_alternative<Empty>(reg)) {
            return Universe{};
        }
    ...
    if (bbox.xmin > universe.xmin || (bbox.xmin == universe.xmin && bbox.lopen)) {
        fragments.emplace_back(
            universe.xmin, bbox.xmin, bbox.ymin, bbox.ymax,
            false, !bbox.lopen, bbox.topen, bbox.bopen);
    }

    // Get right fragment
    if (bbox.xmax < universe.xmax || (bbox.xmax == universe.xmax && bbox.ropen)) {
        fragments.emplace_back(
            bbox.xmax, universe.xmax, bbox.ymin, bbox.ymax,
            !bbox.ropen, false, bbox.topen, bbox.bopen);
    }

    // Get top fragment
    if (bbox.ymin > universe.ymin || (bbox.ymin == universe.ymin && bbox.topen)) {
        fragments.emplace_back(
            universe.xmin, universe.xmax, universe.ymin, bbox.ymin,
            false, false, false, !bbox.topen);
    }

    // Get bottom fragment
    if (bbox.ymax < universe.ymax || (bbox.ymax == universe.ymax && bbox.bopen)) {
        fragments.emplace_back(
            universe.xmin, universe.xmax, bbox.ymax, universe.ymax,
            false, false, !bbox.bopen, false);
    }
...
struct TopoSimplify {
    struct Interval {
        ...
        constexpr auto is_overlapping(const Interval& other) const -> bool {
    ...
    auto get_all_xs(const std::set<BBox, BBoxOrder>& rects) const -> std::set<double> {
    ...
    auto get_y_ranges(const std::set<BBox, BBoxOrder>::const_iterator& start,
    ...
    auto operator()(const Union& region) -> Region {
        ...
        for (size_t i = 0; i + 1 < x_margins_vec.size(); ++i) {
            ...
            while (iter != rects.cend() && iter->xmax < x1) {
                ++iter;
            }
    ...
    return std::visit(
        [](const auto& r) -> Region {
            if constexpr (std::is_same_v<std::decay_t<decltype(r)>, Empty>) {
                ...
                Union ret;
                for (const auto& bbox : r) {
                    ret.insert(BBox{bbox.xmin, bbox.xmax, bbox.ymin, bbox.ymax,
                                    true, true, true, true});
                }
                return ret;
            }
...
auto intersect(const Region& lhs, const Region& rhs) -> Region {
    ...
    if (std::holds_alternative<Universe>(lhs)) {
        return rhs;
    }
    if (std::holds_alternative<Universe>(rhs)) {
        return lhs;
    }
    ...
    if (auto* union_lhs = std::get_if<Union>(&lhs)) {
        ...
        if (union_rhs) {
            return intersection_of(*union_rhs, std::get<BBox>(lhs));
        }
    }
...
auto union_of(const Region& lhs, const Region& rhs) -> Region {
    ...
    if (std::holds_alternative<Empty>(rhs)) {
        return lhs;
    }
    ...
    if (auto* union_lhs = std::get_if<Union>(&lhs)) {
        ...
        auto* union_rhs = std::get_if<Union>(&rhs);
        if (union_rhs) {
            return union_of(*union_rhs, std::get<BBox>(lhs));
        }
    }
